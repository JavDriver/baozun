...
var MAC={
	...
	'History': {
		...
		'Clear': function(){
			...
			$('#history_box').html('<li class="hx_clear"  style= "margin-top:3px;margin-left:10px;">已清空观看记录。</li>');
		},	
		'Show': function(){
			$('#history_box').show();
		},
		...
		'Create': function($id){
			...
			if(jsondata.length > 0){
				...
				html +='<dd class="hide"  style= "margin-top:3px;margin-left:10px;">暂无观看记录</dd>';
			}
			html += '</dl>';
			$('#'+$id).after(html);
			...
			$('#history_box').css({'left':'-123px','top':(position.top+h)});
		},	
